  METHOD if_http_service_extension~handle_request.
    ...
    ELSE.
*birden fazla banka seçilemesin kontrolü
      DATA(lt_bankinternalid) = ms_request-invoicedata.
      SORT lt_bankinternalid BY bankinternalid.
      DELETE ADJACENT DUPLICATES FROM lt_bankinternalid COMPARING bankinternalid.
      IF ( lines( lt_bankinternalid ) > 1 ).
        APPEND VALUE #( id = 'YDBS_MC' type = mc_error number = 026 ) TO ms_response-messages.
      ELSE.
*limiti var mı kontrolü
        SELECT limit~* FROM @ms_request-invoicedata AS invoicedata
                 INNER JOIN ydbs_t_limit AS limit ON limit~companycode = invoicedata~companycode
                                                 AND limit~bankinternalid = invoicedata~bankinternalid
                                                 AND limit~customer = invoicedata~customer
